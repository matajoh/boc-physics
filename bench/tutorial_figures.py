...
so the static-first z-ordering matches exactly what a reader sees on screen. The
hero shot is the generator-fed pachinko scene. Bodies are coloured with the same
vivid HLS hue the live window and the pachinko generator use, so every figure
shares one palette. The shower draws from the seeded Matrix PRNG, so each figure
reproduces the same frame every run.
...
FIGURES = (
    FigureSpec("pachinko_hero", "pachinko", 8, 320, Resolution(900, 900), 0, 0),
)
