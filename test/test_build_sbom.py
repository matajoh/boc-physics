...
The script is not part of the installed bocphysics package — it lives
at ``scripts/`` and is invoked by the release workflow to inject a
PEP 770 SBOM into the built wheel. ``scripts/`` is added to
``sys.path`` for the test session via ``pythonpath`` in
``pyproject.toml``'s ``[tool.pytest.ini_options]`` block, so it
imports like any other third-party module here.
"""
...
def _build_probe_wheel(path: Path) -> None:
    ...
    entries = [
        (f"{DIST}/__init__.py", b'__version__ = "0.3"\n'),
...
def test_build_sbom_document_minimal_shape():
    ...
    assert root["name"] == "bocphysics"
    assert root["version"] == "0.3"
    assert root["purl"] == "pkg:pypi/bocphysics@0.3"
    assert root["bom-ref"] == "pkg:pypi/bocphysics@0.3"
    assert root["licenses"] == [{"license": {"id": "MIT"}}]
    assert "properties" not in root

    assert doc["components"] == []
    assert doc["dependencies"] == [
        {"ref": "pkg:pypi/bocphysics@0.3", "dependsOn": []}
    ]
...
def test_build_sbom_document_attaches_properties_when_supplied():
    ...
    doc = build_sbom.build_sbom_document(
        ...
        wheel_filename="bocphysics-0.3-cp314-cp314-linux_x86_64.whl",
    )
    ...
    assert (
        ...
        == "bocphysics-0.3-cp314-cp314-linux_x86_64.whl"
    )
...
def test_inject_sbom_round_trip(tmp_path: Path) -> None:
    ...
    sbom_arc = f"{DIST_INFO}/sboms/bocphysics.cdx.json"
...
def test_cli_inject_copy_to_leaves_original_alone(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(copied, "r") as wheel:
        names = wheel.namelist()
        assert f"{DIST_INFO}/sboms/bocphysics.cdx.json" in names
        sbom_doc = json.loads(
            wheel.read(f"{DIST_INFO}/sboms/bocphysics.cdx.json")
        )
...
def test_build_sbom_document_serial_changes_with_inputs() -> None:
    ...
    *should* be distinguishable have distinct serial numbers).
    ...
    doc_c = build_sbom.build_sbom_document(
        **{**base, "wheel_filename": "bocphysics-0.3-cp314-cp314-win_amd64.whl"}
    )
...
def _build_probe_wheel_with_dir_entries(path: Path) -> None:
    ...
    in the repaired wheel — ``bocphysics/``, ``bocphysics/include/``,
    ``bocphysics-X.Y.Z.dist-info/`` and so on. ``zipfile.writestr(str, ...)``
