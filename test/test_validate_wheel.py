...
bocphysics-0.7.0 wheel: a wheel whose RECORD lists ZIP directory entries
...
def test_malformed_filename_raises(tmp_path):
    """A basename lacking the two name-version separators is rejected up front."""
    wheel_path = tmp_path / "bad.whl"
    wheel_path.touch()

    with pytest.raises(validate_wheel.InvalidWheelFilenameError):
        validate_wheel.validate_wheel_file(wheel_path)


def test_main_reports_malformed_filename(tmp_path, capsys):
    """main() reports a malformed wheel filename as a per-file FAIL and returns 1."""
    wheel_path = tmp_path / "bad.whl"
    wheel_path.touch()
    ...
    assert "malformed wheel filename" in err
...
def test_valid_entry_points_pass(tmp_path):
    ...
    entries = _wheel_metadata_entries() + [
        (
            ...
            b"[console_scripts]\nbocphysics-tool = bocphysics.cli:main\n",
        ),
    ]
