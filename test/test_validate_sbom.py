...
``release.yml``. These tests pin the structural invariants the
...
longer match the contract bocphysics ships.
...
def test_validate_sbom_file_accepts_round_trip(tmp_path: Path):
    sbom = tmp_path / "bocphysics.cdx.json"
...
def test_root_component_bom_ref_and_purl_must_agree(tmp_path: Path):
    doc = _good_doc()
    doc["metadata"]["component"]["purl"] = "pkg:pypi/bocphysics@0.6.1"
    with pytest.raises(validate_sbom.ValidationError, match="purl"):
        validate_sbom.validate_sbom_document(doc)


def test_root_component_purl_must_be_pypi_bocphysics():
    doc = _good_doc()
    doc["metadata"]["component"]["bom-ref"] = "pkg:npm/bocphysics@0.3"
    doc["metadata"]["component"]["purl"] = "pkg:npm/bocphysics@0.3"
...
def test_main_accepts_directory_and_returns_zero_on_success(
    ...
    for tag in ("cp310", "cp314"):
        sbom = sboms / f"bocphysics-0.3-{tag}.cdx.json"
        sbom.write_text(json.dumps(_good_doc()), encoding="utf-8")
    ...
    assert "bocphysics-0.3-cp310.cdx.json" in out.out
    assert "bocphysics-0.3-cp314.cdx.json" in out.out
...
def test_good_doc_helper_is_deep_copyable():
    ...
    assert b["metadata"]["component"]["name"] == "bocphysics"
