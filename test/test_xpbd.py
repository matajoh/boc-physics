...
def run_drop(frames=180):
    ...
    for _ in range(frames):
        jacobi.solve_group_substep(INELASTIC, [box], pairs, GRAVITY, SUB_DT, 4)
    return box, floor
...
def measured_restitution(restitution, frames=150):
    ...
    impact = rebound = 0.0
    for _ in range(frames):
        jacobi.solve_group_substep(physics, [ball], pairs, GRAVITY, SUB_DT, 4)
...
def test_random_pile_stays_finite_and_bounded(seed):
    ...
    for uid, body in enumerate([floor] + boxes):
        body.uid = uid
    for _ in range(180):
        jacobi.solve_group_substep(FRICTION, boxes, pairs, GRAVITY, SUB_DT, 4)
...
def test_broad_box_never_rejects_a_real_overlap(seed):
    ...
    a, b = build(), build()
    a.uid, b.uid = 0, 1
    box_a = xpbd._broad_box(a)
    box_b = xpbd._broad_box(b)
