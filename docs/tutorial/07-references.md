...
<a id="seabra-2023"></a>
**Seabra 2023** — Miguel Seabra, Daniel Simões Lopes, João Madeiras Pereira,
*Survey of Rigid Body Simulation with Extended Position Based Dynamics*, arXiv
2311.09327, 2023. [PDF](https://arxiv.org/pdf/2311.09327). Benchmarks position-based
rigid bodies against PhysX and Havok and pins down the one place they fall short:
**stable stacks**. It traces the resting-stack *drift* to sequential (Gauss–Seidel)
contact resolution — an off-centre correction applies a rotation as well as a
translation, that rotation penetrates the opposite corner, and the next correction
overcorrects — and notes that a **Jacobi** solve, which averages each body's
corrections so the opposing rotations cancel, holds stacks markedly better (though
still not perfectly). This engine reproduces both the drift and the Jacobi
mitigation; see [Chapter 6](06-future-work.md).
