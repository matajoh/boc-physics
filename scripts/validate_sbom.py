...
bocphysics ships one CycloneDX 1.6 JSON SBOM per wheel under
``<dist>-<version>.dist-info/sboms/bocphysics.cdx.json``. The release
workflow extracts the embedded SBOM and runs this validator over the
extracted set as a defence-in-depth check: if a refactor of
``build_sbom.py`` produces something that an SCA tool would silently
ignore (e.g. missing ``bomFormat`` or wrong ``specVersion``), the
verification step fails before the wheel is ever published.

The validator is deliberately stdlib-only — it asserts the structural
invariants that bocphysics itself commits to, not the full CycloneDX
1.6 schema. The full schema check is handed off to a real SCA
consumer. The combination gives us:

* fast, dependency-free shape verification (this script), and
* third-party tool consumability proof (downstream SCA).
...
EXPECTED_PURL_PREFIX = "pkg:pypi/bocphysics@"
...
def validate_sbom_document(doc: Any) -> None:
    """Assert that ``doc`` is one of bocphysics's PEP 770 CycloneDX 1.6 SBOMs.
...
def validate_sbom_file(path: Path) -> None:
    ...
    :raises ValidationError: If the file is not valid JSON or fails any
        of bocphysics's PEP 770 invariants.
    """
    try:
...
def _expand_inputs(inputs: Iterable[str | os.PathLike[str]]) -> list[Path]:
    ...
    ``sboms/`` directory produced by the release workflow.
    """
    files: list[Path] = []
    for item in inputs:
        p = Path(item)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Structurally validate bocphysics's PEP 770 CycloneDX 1.6 SBOMs. "
