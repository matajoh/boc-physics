...
`simulation = bocphysics:main`) and as a teaching aid for learning to program
with BOC: source files double as lecture notes and a revision aid, so
...
| `@when(*cowns)` | Decorator. Schedules the function as a behavior with exclusive access to the listed cowns. The first N parameters bind to the N cowns; any **extra** values must be passed as trailing parameters with defaults (`x=x`). |
...
- **The first N parameters must match the N `@when` cowns.** A behavior runs
  in another interpreter, so any extra value it needs must be a **trailing
  parameter with a default** (`def b(c, i=i)` captures `i`; `def b(c, x=y)`
  captures `y`). The leading cown parameters carry no defaults.
- **A behavior cannot close over a free variable.** Referencing a name from
  the enclosing scope (a loop variable, a helper arg) that is not a parameter
  raises `SyntaxError` at `@when` decoration time, naming the offending
  variable. Capture it explicitly as a trailing default instead: `def _(c,
  x=x)`. This is the opposite of the old transpiler behavior — closures are
  now rejected, and `x=x` is the required fix, not a mistake.
...
- `src/bocphysics/` — the package source. Notable modules:
  ...
  - `simulation.py` — top-level `Simulation` class; the pyglet loop and the
...
- `pyproject.toml` — `setuptools` build, `bocpy` and `pyglet` runtime deps,
...
flake8 src test bench        # lint package, tests, and benchmarks (must be rc=0)
```
...
Every code comment must be at most one line and no longer than 120 characters.
Do not write multi-line comment blocks without explicit approval from the user.
